...
function Ship(ship){
	Ship_singleton.getInstance(ship);
	console.log(ship);
	var shape, firearm;
	var move_right = move_left = move_up = move_down = false;
	var last_mouse_position = {x:0, y:0};
...
Ship.prototype.renderShip = function(){
	...
	this.ship.shape.components.forEach(function(component){
		var shape = new createjs.Shape();
		shape.graphics.bf(loader.getResult(this.ship.shape.file)).drawRect(component.crop_x,component.crop_y,component.width,component.height);
...
Ship.prototype.renderHealthBar = function(){
	...
	this.health_bar.graphics.beginFill("#00CC00").drawRect( -this.ship.shape.radius, -this.ship.shape.radius, this.ship.shape.radius * 2, this.ship.shape.radius/5);
	this.damage_bar.graphics.beginFill("#CC0000").drawRect(this.ship.shape.radius * 2 / this.health_max * this.health - this.ship.shape.radius, -this.ship.shape.radius, this.ship.shape.radius * 2 * (this.health_max - this.health) / this.health_max, this.ship.shape.radius / 5);
...
Ship.prototype.renderText = function(){
	...
	this.level_up_text.regY = this.ship.shape.radius * 2;
...
Ship.prototype.isHit = function(bullet){
	return (Math.pow(bullet.x - this.container.x, 2) + Math.pow(bullet.y - this.container.y, 2) < Math.pow(this.ship.shape.radius + bullet.radius, 2));
}
...
Ship.prototype.damaged = function(bullet){
	...
	this.damage_bar.graphics.c().beginFill("#CC0000").drawRect(this.ship.shape.radius * 2 / this.health_max * this.health - this.ship.shape.radius, -this.ship.shape.radius, this.ship.shape.radius * 2 * (this.health_max - this.health) / this.health_max, this.ship.shape.radius / 5);	
